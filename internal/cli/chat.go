...
func runChatLoop(ctx context.Context, plan *chat.Plan, client chatBackend, sess *chat.Session, stream, verbose bool, in io.Reader, out io.Writer, backendDone <-chan struct{}, backendErr func() error, backendLog func() string, st chatStyle, knowledgeBases []*knowledge.Base, backendOrigin string) error {
	...
	// Scene images are generated by a child process, so its result cannot be
	// printed from the goroutine that waits on it — the loop owns this writer.
	// It arrives on a channel the input select drains instead, and is dropped
	// rather than blocking a generation nobody is waiting on.
	image := newChatImage(plan, sess)
	imageResults := make(chan string, 8)
	fireImage := func(reply string) {
		if !image.on() || strings.TrimSpace(reply) == "" {
			return
		}
		// Built here, on the loop's own goroutine, because it reads the session;
		// the goroutine below then shares nothing with the conversation.
		req := chat.SceneRequest(sess)
		img := image
		go func() {
			g := img.generate(ctx, client, req, reply)
			var lines []string
			if g.scene != "" {
				lines = append(lines, st.dim("scene: "+g.scene))
			}
			if g.err != nil {
				lines = append(lines, st.errorText("image: ")+g.err.Error())
			} else if last := lastLine(g.out); last != "" {
				lines = append(lines, st.dim("image: "+last))
			}
			if len(lines) == 0 {
				return
			}
			select {
			case imageResults <- strings.Join(lines, "\n"):
			default:
			}
		}()
	}
	...
	for {
		...
		if action, handled := handleSlashCommand(out, sess, &image, msg); handled {
			switch action {
			case slashReset:
				// A reset clears the transcript, including the seeded scene;
				// store that immediately so quitting right after a reset does
				// not resume the conversation just discarded, then replay the
				// opening so the character starts over rather than on a blank
				// stage.
				save()
				if seedOpening() {
					return nil
			...
			case slashImage:
				// Redraw the scene already on screen, so a changed setting can
				// be judged against the reply it will apply to.
				fireImage(lastAssistantReply(sess))
			case slashNone:
			}
			continue
		}
...
// slashAction is what the caller must do after a local command was handled.
// Reset and image are mutually exclusive — they belong to different commands —
// so one value says which, rather than a bool per command.
type slashAction int

const (
	slashNone  slashAction = iota
	slashReset             // replay the opening scene
	slashImage             // generate from the reply already on screen
)

// handleSlashCommand executes local commands. Slash commands are never sent to
// the model. It returns the follow-up the caller owes and whether the message
// was a command at all.
//
...
func handleSlashCommand(out io.Writer, sess *chat.Session, img *chatImage, msg string) (slashAction, bool) {
	if isImageCommand(msg) {
		status, fire := img.set(msg, sess)
		fmt.Fprintf(out, "(%s)\n", status)
		if err := sess.Save(); err != nil {
			fmt.Fprintf(out, "warning: %v\n", err)
		}
		if fire {
			return slashImage, true
		}
		return slashNone, true
	}
	switch msg {
	case "/reset":
		sess.Reset()
		fmt.Fprintln(out, "(conversation reset)")
		return slashReset, true
	case "/context":
		info := sess.Context()
	...
	if strings.HasPrefix(msg, "/") {
		fmt.Fprintf(out, "unknown command %q (try /reset, /context, /image)\n", msg)
		return slashNone, true
	}
...
func printStartupSummary(out io.Writer, plan *chat.Plan, st chatStyle, restored int, backendOrigin string) {
	...
	fmt.Fprintln(out, st.dim("Ctrl-C or Ctrl-D to quit · /reset · /context · /image"))
}
