...
func (e ArtifactEdges) OwnerOrErr() (*User, error) {
	if e.Owner != nil {
		return e.Owner, nil
...
func (e ArtifactEdges) VersionsOrErr() ([]*Version, error) {
	if e.loadedTypes[1] {
...
func (*Artifact) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case artifact.FieldID, artifact.FieldNamespace, artifact.FieldName:
			values[i] = new(sql.NullString)
		case artifact.FieldCreatedAt, artifact.FieldUpdatedAt:
			values[i] = new(sql.NullTime)
		case artifact.ForeignKeys[0]: // user_artifacts
			values[i] = new(sql.NullString)
		default:
...
func (a *Artifact) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case artifact.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field user_artifacts", values[i])
			} else if value.Valid {
				a.user_artifacts = new(string)
				*a.user_artifacts = value.String
			}
		default:
...
// QueryOwner queries the "owner" edge of the Artifact entity.
func (a *Artifact) QueryOwner() *UserQuery {
	return NewArtifactClient(a.config).QueryOwner(a)
}

// QueryVersions queries the "versions" edge of the Artifact entity.
func (a *Artifact) QueryVersions() *VersionQuery {
	return NewArtifactClient(a.config).QueryVersions(a)
}
