...
func (uq *UserQuery) QueryArtifacts() *ArtifactQuery {
	query := (&ArtifactClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withArtifacts:  uq.withArtifacts.Clone(),
...
func (uq *UserQuery) WithArtifacts(opts ...func(*ArtifactQuery)) *UserQuery {
	query := (&ArtifactClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withArtifacts = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withArtifacts; query != nil {
		if err := uq.loadArtifacts(ctx, query, nodes,
			func(n *User) { n.Edges.Artifacts = []*Artifact{} },
			func(n *User, e *Artifact) { n.Edges.Artifacts = append(n.Edges.Artifacts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadArtifacts(ctx context.Context, query *ArtifactQuery, nodes []*User, init func(*User), assign func(*User, *Artifact)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Artifact(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ArtifactsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_artifacts
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_artifacts" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_artifacts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
