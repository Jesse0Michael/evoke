...
func (vq *VersionQuery) QueryArtifact() *ArtifactQuery {
	query := (&ArtifactClient{config: vq.config}).Query()
...
func (vq *VersionQuery) Clone() *VersionQuery {
	...
	return &VersionQuery{
		...
		withArtifact: vq.withArtifact.Clone(),
...
func (vq *VersionQuery) WithArtifact(opts ...func(*ArtifactQuery)) *VersionQuery {
	query := (&ArtifactClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withArtifact = query
	return vq
}
...
func (vq *VersionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Version, error) {
	...
	if query := vq.withArtifact; query != nil {
		if err := vq.loadArtifact(ctx, query, nodes, nil,
			func(n *Version, e *Artifact) { n.Edges.Artifact = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *VersionQuery) loadArtifact(ctx context.Context, query *ArtifactQuery, nodes []*Version, init func(*Version), assign func(*Version, *Artifact)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Version)
	for i := range nodes {
		if nodes[i].artifact_versions == nil {
			continue
		}
		fk := *nodes[i].artifact_versions
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "artifact_versions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
